...
import { ArrowLeft, CheckCircle2, Play, Plus, Minus, ArrowRight, Box, Grid3X3, Sun, Layers, Film, Pause, Volume2, VolumeX } from 'lucide-react';
import { Page } from '../types';
import QuoteForm from './QuoteForm';

// Dynamically import all featured works images (all formats)
const featuredImages = import.meta.glob('/public/assets/images/exterior/featured-works/*.{webp,jpg,jpeg,png}', { eager: true, query: '?url', import: 'default' }) as Record<string, string>;
const galleryImages = Object.values(featuredImages);

// Import all lighting scenario images
const lightingImages = import.meta.glob('/public/assets/images/exterior/lighting/*.{webp,jpg,jpeg,png}', { eager: true, query: '?url', import: 'default' }) as Record<string, string>;
...
const landscapeImages = import.meta.glob('/public/assets/images/exterior/landsacpe/*.{png,jpg,jpeg,webp}', { eager: true, query: '?url', import: 'default' }) as Record<string, string>;
...
const animationVideos = import.meta.glob('/public/assets/images/exterior/animation/*.mp4', { eager: true, query: '?url', import: 'default' }) as Record<string, string>;
const animationGallery = Object.values(animationVideos);

// Import carousel images for Still Renderings
const carouselImages = import.meta.glob('/public/assets/images/exterior/carousel/*.{webp,jpg,jpeg,png}', { eager: true, query: '?url', import: 'default' }) as Record<string, string>;
...
const VideoCarousel: React.FC<{ videos: string[]; title?: string }> = ({ videos, title }) => {
    const [currentIndex, setCurrentIndex] = useState(0);
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                        {assetCards.map((card, i) => (
                            <div key={i} className="group">
                                ...
                                ) : card.useVideoCarousel ? (
                                    <VideoCarousel videos={animationGallery} title="Animation" />
                                ) : (
